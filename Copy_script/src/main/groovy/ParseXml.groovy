...
class ParseXml {
 ...
 void create_object_vm(){
     ...
     println("Generate connection object. ")
    ...
    void add_file_repo_objects(){
        println("Add file to local repository.")
        for(def i=0;i<list_data_connect.size();++i){

            if(list_data_connect.get(i).getVm_status()==true){


                create_git_local_reposytory_objects("true")

                if(access == list_data_connect.get(i).getVm_status()){ clear_comits_git(); access=false}

                println("Create  file to local repository:" + data_git_repo.getRepository_path() + "save_temporar_repo_git\\" + data_git_repo.getRepository_name() + "\\" + list_data_connect.get(i).getVm_name())
                ...
                for (def j = 0; j < list_data_connect.get(i).local_path_file.size(); ++j) {
                    ...
                    if (execute_cmd("dir /b " + list_data_connect.get(i).local_path_file.get(j)) != "") {
                        ...
                        println("In this path " + list_data_connect.get(i).local_path_file.get(j) + " don't exist file " + list_data_connect.get(i).file_name.get(j))
                    }
                }
    ...
    void clear_comits_git(){

        def counts=execute_cmd("cd /D "+data_git_repo.getRepository_path()+"\\save_temporar_repo_git\\"+data_git_repo.getRepository_name()+"\\ & git rev-list --all --count")

      if(counts != 1) {

           def file_commits = new File("src/main/resources/source_bat/commits_clear.bat")

            def file_save_commits = file_commits.getText()
           file_save_commits=file_save_commits.replace('${directory}',data_git_repo.getRepository_path())
           file_save_commits=file_save_commits.replace('${local_repository}',data_git_repo.getRepository_name())
           file_save_commits=file_save_commits.replace('${delete_HEAD}',"git reset --hard HEAD~" + (counts.toInteger()-2))


            def file_temporar_commits = new File("src/main/resources/source_bat/commits_clear_1.bat")
            file_temporar_commits.write(file_save_commits)


            execute_cmd(file_temporar_commits.getAbsolutePath().toString())
            file_temporar_commits.delete()
       }
